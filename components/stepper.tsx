...
export default function () {
    ...
    const [step, setStep] = useLocalStorage('stepper_step', 0)
    const [offset, setOffset] = useLocalStorage('stepper_offset', 0)
    const m = 60
    ...
    const goToStep = async (number: number) => {
        ...
        synth?.speak(new SpeechSynthesisUtterance(`${(offset + instruction.nailIndex) % instructions.map.length}. ${RotationDirection[instruction.direction]}`))
    }
    const first = async () => {
        goToStep(0)
    }
    ...
    const nailIdx: number = instructions.steps.length > 1
        ? instructions.steps[step + 1].nailIndex
        : 0

    const nail: Nail = instructions.steps.length > 1
        ? instructions.map[instructions.steps[step + 1].nailIndex]
        : new Nail
    ...
    const getPosition = (s: number) => {
        ...
        var v: IVector2D = Point2D.substract(n2.position, n1.position)
        ...
        if (s1.direction == s2.direction) {
            const a: number = Math.acos((n1.diameter + n2.diameter) / l) * (s1.direction == RotationDirection.ClockWise ? -1 : 1);
            r1 = Point2D.add(n1.position, Vector2D.scale(Vector2D.rotate(v, a), n1.diameter / 2))
            r2 = Point2D.add(n2.position, Vector2D.scale(Vector2D.rotate(v, a), n2.diameter / 2))
        }
        else {
            const a: number = Math.acos((n1.diameter - n2.diameter) / l) * (s1.direction == RotationDirection.ClockWise ? -1 : 1);
            r1 = Point2D.add(n1.position, Vector2D.scale(Vector2D.rotate(v, a), n1.diameter / 2))
            r2 = Point2D.add(n2.position, Vector2D.scale(Vector2D.rotate(Vector2D.scale(v, -1), a), n2.diameter / 2))
        }
    ...
    return (
        <React.Fragment>
            <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', width: '100%' }}>
                ...
                <Box>
                    <IconButton disabled={step == 0} onClick={first}>
                        <FirstPage />
                    </IconButton>
                    <IconButton disabled={step == 0} onClick={prev}>
                        <ChevronLeft />
                    </IconButton>
                    <span style={{
                    ...
                    <TextField value={step + 1} onChange={(e) => goToStep(Number.parseInt(e.target.value) - 1)} />
                         / {instructions.steps.length - 1}
                    </span>
                    <IconButton disabled={step == instructions.steps.length - 2} onClick={next}>
                        <ChevronRight />
                    </IconButton>
                    <IconButton disabled={step == instructions.steps.length - 2} onClick={last}>
                        <LastPage />
                    </IconButton>
                </Box>
                <Box>
                    <TextField value={offset} onChange={(e) => setOffset(Number.parseInt(e.target.value))} />
                </Box>
                <Box>
                    {JSON.stringify(nail)}
                </Box>
                <Box sx={{
                    ...
                    <svg viewBox={`${minX} ${minY} ${maxX - minX} ${maxY - minY}`} preserveAspectRatio="xMidYMid" height='100%' width='100%' >
                        <text {...getTextPosition()} >{(offset + nailIdx) % instructions.map.length}</text>
                        <linearGradient
                            id="gradient_0000" >
                        ...
                        }
                        {
                            ...
                            && <g> <linearGradient
                                ...
                                <path
                                    fill="red"
                                    transform={`translate(${nail.position.x}, ${nail.position.y}) rotate(${getArrowAngle(step)})`}
                                    d={
                                        instructions.steps[step + 1].direction == RotationDirection.ClockWise
