...
import { Line, LineHelper, PointHelper, VectorHelper } from "@/tools/geometry";
...
export default function ({ projectId }: Options) {
    ...
    React.useEffect(() => {
        Action.try(async () => {
            ...
            setState({
                ...instructions,
                svgInfo: {
                    minX: Math.min(...instructions.nails.map(n => n.position.x - n.diameter / 2)) - SVG_MARGIN,
                    maxX: Math.max(...instructions.nails.map(n => n.position.x + n.diameter / 2)) + SVG_MARGIN,
                    minY: Math.min(...instructions.nails.map(n => n.position.y - n.diameter / 2)) - SVG_MARGIN,
                    maxY: Math.max(...instructions.nails.map(n => n.position.y + n.diameter / 2)) + SVG_MARGIN,
                }
            })
        })
    }, [])
    ...
    function getTextPosition() {
        if (state!.steps.length > 1) {
            return PointHelper.add(nail.position, VectorHelper.scale(VectorHelper.normalize(PointHelper.substract(nail.position, { x: (state!.svgInfo.minX + state!.svgInfo.maxX) / 2, y: (state!.svgInfo.minY + state!.svgInfo.maxY) / 2 },)), 20))
        }
        else return {
            x: 100,
            y: 100,
    ...
    const nailIdx: number = state.steps.length > 1
        ? state.steps[settings.step + 1].nailIndex
        : 0

    const nail: Nail = state.steps.length > 1
        ? state.nails[state.steps[settings.step + 1].nailIndex]
        : {
            diameter: 1.8,
            position: { x: 0, y: 0 }
        }
    ...
    return (
        <React.Fragment>
            ...
            <svg viewBox={`${state.svgInfo.minX} ${state.svgInfo.minY} ${state.svgInfo.maxX - state.svgInfo.minX} ${state.svgInfo.maxY - state.svgInfo.minY}`}
                ...
                }
                {
                    ...
                    && <g>
                        <text {...getTextPosition()} >{(settings.offset + nailIdx) % state.nails.length}</text>
                        <linearGradient
