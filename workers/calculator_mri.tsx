...
function calculate(project: IProject, imageDatas: Array<Uint8ClampedArray>, threads: Array<boolean>): Array<IStep> {
    ...
    for (let i = 0; i < project.threads.length; i++) {
        ...
        const linesWeights: WeightLine[][][] = lines.map((a0: ILineInfo[][]) =>
            a0.map((a1: ILineInfo[]) =>
                a1.map((l: ILineInfo) => {
                    const pLine = PixelLine.get({
                    ...
                    return {
                        line: l,
                        weight: pLine.points.reduce((sum: number, p: IWeightPoint2D) => sum + p.weight * target[p.x][p.y], 0)
                    }
                })
            )
        )
        ...
        do {
            ...
            const arr = linesWeights[nail!.nailIndex][nail!.direction]

            for (let j = 0; j < arr.length; j++) {

                if (arr[j].weight <= indicator)
                    continue

                indicator = arr[j].weight
                arr[j].weight = 0
                nextNail = {
                    nailIndex: arr[j].line.n1Idx,
                    direction: arr[j].line.r1
                }
            }
